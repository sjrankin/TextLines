...
class DoubleCell: UITableViewCell, UITextFieldDelegate, CellProtocol
{
    ...
    func InitializeUI()
    {
        ...
        TextField = UITextField(frame: CGRect(x: CurrentWidth - (100 + 10), y: DoubleCell.CellHeight / 2 - 30 / 2,
                                              width: 100, height: 30))
    ...
    func textFieldDidEndEditing(_ textField: UITextField)
    {
        ...
        if let Result = textField.text
        {
            ...
            {
                let FinalValue = ForOutput?(DValue) ?? DValue
                Settings.SetDouble(Key, FinalValue)
            }
        }
    }
    ...
    var ForInput: DoubleConverter? = nil
    var ForOutput: DoubleConverter? = nil
