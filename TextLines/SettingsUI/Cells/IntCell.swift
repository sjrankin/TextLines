...
class IntCell: UITableViewCell, UITextFieldDelegate
{
    ...
    func InitializeUI()
    {
        ...
        TextField = UITextField(frame: CGRect(x: CurrentWidth - (100 + 10), y: IntCell.CellHeight / 2 - 30 / 2,
                                              width: 100, height: 30))
        TextField.delegate = self
        TextField.borderStyle = .roundedRect
        TextField.keyboardType = .numberPad//.default
    ...
    func textFieldDidEndEditing(_ textField: UITextField)
    {
        ...
        {
            if let IVal = Int(Result)
            {
                Settings.SetInt(Key, IVal)
            }
        }
    }
